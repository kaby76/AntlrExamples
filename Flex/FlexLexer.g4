...
IStartCodeBlock : Ws { input.LA(-1) == '\n' } -> skip ;
Icomment : '/*' { input.LA(-1) == '\n' } -> pushMode(COMMENT), skip ;
Ilinedir : '#' OptWs 'line' Ws -> pushMode(LINEDIR), skip ;
SCDECL : '%s' Id? ;
XSCDECL : '%x' Id? ;
IEndCodeBlock : '%{' .* Nl -> skip ;
ITop : '%top' Ws '{' Ws Nl -> pushMode(CODEBLOCK_MATCH_BRACE) ;
SECTEND:   '%%' -> type(SECTEND), pushMode(SECT2PROLOG);
NAME : Id ;
TOK_OPTION : '%option' -> type(TOK_OPTION), pushMode(OPTION);
NLOPTION : Nl -> popMode ;
IDiscard : Ws -> skip ;
