...
namespace Antlr
{
    ...
    static class Output
    {
        ...
        public static StringBuilder OutputTokens(this CommonTokenStream stream, Lexer lexer)
        {
            ...
            {
                sb.AppendLine("Token " + token.TokenIndex + " " + token.Type + " " + "channel " + lexer.ChannelNames[token.Channel] + " " + Output.PerformEscapes(token.Text));
            }
            return sb;
        }
        ...
        private static void ParenthesizedAST(this IParseTree tree, StringBuilder sb, CommonTokenStream stream, Lexer lexer, int level = 0)
        {
            ...
            if (tree as TerminalNodeImpl != null)
            {
                ...
                if (inter != null)
                    foreach (var t in inter)
                    {
                        ...
                        sb.AppendLine("( " + lexer.ChannelNames[t.Channel] + " text=" + t.Text.PerformEscapes());
                    }
                StartLine(sb, tree, stream, level);
            ...
            for (int i = 0; i < tree.ChildCount; ++i)
            {
                ...
                c.ParenthesizedAST(sb, stream, lexer, level + 1);
            }
            if (level == 0)
            {
